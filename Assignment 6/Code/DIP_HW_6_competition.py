...
def Shrink(X, shrinkArg):
        ...
        Xmin = block_reduce (X, block_size = (1,2,2,1), func=np.min)
        # Xmax = block_reduce (X, block_size = (1,2,2,1), func=np.max)
        # Xmean = block_reduce (X, block_size = (1,2,2,1), func=np.mean)
        return Xmin
...
svm.fit(Lag_, label_train)

y_pred = svm.predict(Lag_test)
